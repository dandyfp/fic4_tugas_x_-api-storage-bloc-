...
class NetworkManager {
  Future<ProductModel> fetchProduct() async {
    final response = await http.get(Uri.parse('https://api.escuelajs.co/api/v1/products/24'));
  ...
  Future<List<Album>> fetchAllAlbum() async {
    ...
    if (response.statusCode == 200) {
      List<Album> listData = List<Album>.from(json.decode(response.body).map((x) => Album.fromJson(x)));
      return listData;
    } else {
      throw Exception('Failed to load album');
    }
